...
from .backbone import build_backbone, build_Joiner
...
class HSPN_DETRVAE(nn.Module):
    def __init__(self, joiner, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
            transformer: torch module of the transformer architecture. See transformer.py
        ...
        if joiner is not None:
            self.input_proj = nn.Conv2d(joiner[0].num_channels[-1], hidden_dim, kernel_size=1)
            self.joiner = nn.ModuleList(joiner)
            self.input_proj_robot_state = nn.Linear(state_dim, hidden_dim)
        else:
            # input_dim = 14 + 7 # robot_state + env_state
            self.input_proj_robot_state = nn.Linear(state_dim, hidden_dim)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, vq_sample=None):
        ...
        if self.joiner is not None:
            ...
            all_cam_pos = []
            for cam_id, cam_name in enumerate(self.camera_names):
                features, pos = self.joiner[cam_id](image[:, cam_id])
                features = features[0] # take the last layer feature  因为是list对象，需要取出先
                pos = pos[0]
                all_cam_features.append(self.input_proj(features))
                all_cam_pos.append(pos)
            # proprioception features
...
def build_HSPNvae(args):
    ...
    # backbones = []
    # for _ in args.camera_names:
    #     backbone = build_base_backbone(args)
    #     backbones.append(backbone)
